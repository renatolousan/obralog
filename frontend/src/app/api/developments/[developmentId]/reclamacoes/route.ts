...
export async function GET(request: Request) {
  try {
    ...
    const backendUrl = buildBackendUrl(`/api/reclamacoes/${developmentId}`);
    ...
    console.error("Erro ao buscar reclamacoes do desenvolvimento no backend:", error);
    return NextResponse.json(
      {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Nao foi possivel listar as reclamacoes",
      },
      { status: 500 }
    );
  }
